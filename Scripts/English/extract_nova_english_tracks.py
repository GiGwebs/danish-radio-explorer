...
def main():
    ...
    year_dirs = [
        os.path.join(project_dir, f"Outputs/English/{start_year}"),
        os.path.join(project_dir, f"Outputs/Archive/{start_year}")
    ]
    for year_dir in year_dirs:
        if not os.path.exists(year_dir):
            os.makedirs(year_dir)
    
    # Format the date range for filenames
    date_range = f"{start_date.strftime('%Y-%m-%d')}_to_{end_date.strftime('%Y-%m-%d')}"
    
    # Set up output file paths
    raw_output = os.path.join(project_dir, f"Outputs/Archive/{start_year}/NOVA_English_Raw_{date_range}.csv")
    summary_output = os.path.join(project_dir, f"Outputs/English/{start_year}/NOVA_English_Titles_{date_range}.csv")
    
    # Extract English tracks
    english_tracks = process_nova_data_for_2025()
