...
LOGGER = logging.getLogger("radio_playlist_extractor")
...
def parse_runtime_flags(args):
    """Parse runtime flags for timeout, retries, backoff, and log level.

    Returns:
        tuple: (timeout, retries, backoff, log_level, remaining_args)
    """
    ...
    while i < len(args):
        ...
        if arg.startswith("--timeout="):
            ...
            i += 1
            continue
        if arg in ("--retries", "-r") and i + 1 < len(args):
            try:
        ...
        if arg.startswith("--retries="):
            ...
            i += 1
            continue
        if arg in ("--backoff", "-b") and i + 1 < len(args):
            try:
            ...
            i += 2
            continue
        if arg.startswith("--backoff="):
            try:
                backoff = float(arg.split("=", 1)[1])
            except Exception:
                pass
            i += 1
            continue
        if arg in ("--log-level", "--log", "-v") and i + 1 < len(args):
            log_level = args[i + 1].upper()
            i += 2
            continue
        if arg.startswith("--log-level=") or arg.startswith("--log="):
            log_level = arg.split("=", 1)[1].upper()
            i += 1
            continue
        remaining.append(arg)
        i += 1
    return timeout, retries, backoff, log_level, remaining

def _create_session(retries=DEFAULT_RETRIES, backoff_factor=DEFAULT_BACKOFF_FACTOR):
...
def extract_onlineradiobox_playlist(url, station_name):
    ...
    try:
        ...
        LOGGER.debug("Fetching URL for %s: %s", station_name, fetch_url)
        response = SESSION.get(fetch_url, headers=headers, timeout=DEFAULT_TIMEOUT)
        ...
        if response.status_code == 200:
            ...
            if len(playlist_data) == 0:
                try:
                    ...
                    snapshot_dir = os.path.join(project_dir, "Outputs", "Stations", station_name, "Raw", "HTML_Snapshots")
                    if not os.path.exists(snapshot_dir):
                        os.makedirs(snapshot_dir)
                    snapshot_file = os.path.join(snapshot_dir, f"{station_name}_NoTracks_{current_date}_{datetime.now().strftime('%H%M%S')}.html")
                    with codecs.open(snapshot_file, 'w', 'utf-8') as f:
                        f.write(response.text)
                    print(f"Saved HTML snapshot to: {snapshot_file}")
                except Exception as _e:
                    print(f"Failed to save HTML snapshot: {_e}")
            ...
            try:
                ...
                if not os.path.exists(snapshot_dir):
                    os.makedirs(snapshot_dir)
                snapshot_file = os.path.join(snapshot_dir, f"{station_name}_HTTP{response.status_code}_{current_date}_{datetime.now().strftime('%H%M%S')}.html")
                with codecs.open(snapshot_file, 'w', 'utf-8') as f:
                    f.write(response.text)
                print(f"Saved HTML snapshot to: {snapshot_file}")
            except Exception as _e:
        ...
        LOGGER.exception("Exception when fetching data for %s", station_name)
        ...
        try:
            ...
            if not os.path.exists(snapshot_dir):
                os.makedirs(snapshot_dir)
            snapshot_file = os.path.join(snapshot_dir, f"{station_name}_Exception_{current_date}_{datetime.now().strftime('%H%M%S')}.txt")
            with codecs.open(snapshot_file, 'w', 'utf-8') as f:
                f.write(str(e))
            print(f"Saved exception info to: {snapshot_file}")
        except Exception as _e:
            print(f"Failed to save exception info: {_e}")
        return None
...
def extract_station_playlist(station_name, separate_languages=True):
    ...
    # Distinguish between hard failure (None) and "no playlist" (empty list)
    if playlist_data is None:
        print(f"Failed to retrieve playlist data for {station_name}.")
        return None, None, None, 0

    if isinstance(playlist_data, list) and len(playlist_data) == 0:
        # Mark explicit no-playlist day to inform orchestrator and skip retries
        marker_path = os.path.join(station_dir, "Raw", f"{station_name}_NoPlaylist_{filename_date}.json")
        try:
            ...
            print(f"No tracks found for {station_name}. Wrote no-playlist marker: {marker_path}")
        except Exception as e:
            print(f"Failed to write no-playlist marker for {station_name}: {e}")
        return None, None, None, 0

    # Save raw data
    raw_output = os.path.join(station_dir, "Raw", f"{station_name}_Raw_Playlist_{filename_date}.csv")
    if save_to_csv(playlist_data, raw_output):
        print(f"Raw playlist data saved to: {raw_output}")
    else:
        print("Raw playlist data not saved (no tracks).")
...
def main():
    ...
    # Parse runtime flags (removes them from args)
    timeout, retries, backoff, log_level, args = parse_runtime_flags(args)
    configure_logging(log_level)
    # Apply runtime config
    global DEFAULT_TIMEOUT, SESSION
    DEFAULT_TIMEOUT = timeout
    SESSION = _create_session(retries=retries, backoff_factor=backoff)
    LOGGER.info("Runtime config: timeout=%s, retries=%s, backoff=%s, log_level=%s", timeout, retries, backoff, log_level)
    ...
    if len(args) > 0:
        ...
        if args[0].lower() in ['-h', '--help', 'help']:
            ...
            print("  --timeout N   HTTP timeout in seconds (default: {})".format(DEFAULT_TIMEOUT))
            print("  --retries N   HTTP retry attempts (default: {})".format(DEFAULT_RETRIES))
            print("  --backoff F   Retry backoff factor (default: {})".format(DEFAULT_BACKOFF_FACTOR))
            print("  --log-level L Logging level: DEBUG, INFO, WARNING, ERROR (default: INFO)")
            print("\nExample:")
