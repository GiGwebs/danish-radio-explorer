...
def is_definitely_danish(text):
    ...
    try:
        ...
        # Without Danish characters, require higher confidence
            ...
            # For now, we'll manually check a few known English tracks
            known_english = [
                "Outnumbered", 
...
def process_nova_data_for_2024():
    """Process NOVA Radio data for 2024 (Jan 1 through Dec 31), extracting Danish titles."""
        ...
        if playlist_data:
            ...
            for item in playlist_data:
                if 'nowPlayingTrack' in item and 'nowPlayingArtist' in item and 'eventTime' in item:
                    ...
                    # Check if title is in Danish using our improved detection
                    if title and is_definitely_danish(title):
...
def main():
    print("Starting NOVA Radio Danish titles extraction for 2024...")
    ...
    year_dirs = [
        os.path.join(project_dir, "Outputs/Danish/{}".format(start_year)),
    ...
    raw_output = os.path.join(project_dir, "Outputs/Archive/{}/NOVA_Danish_Raw_{}.csv".format(start_year, date_range))
    summary_output = os.path.join(project_dir, "Outputs/Danish/{}/NOVA_Danish_Titles_{}.csv".format(start_year, date_range))
    
    # Extract Danish tracks
    danish_tracks = process_nova_data_for_2024()
    success = save_to_csv(danish_tracks, raw_output)
    
    if success:
        print("SUCCESS! Raw Danish tracks saved to: {}".format(raw_output))
        
        # Summarize the tracks
        unique_count = summarize_tracks(raw_output, summary_output)
        print("Summarized {} unique Danish tracks to: {}".format(unique_count, summary_output))
        
        # Print a sample of the summarized tracks
        try:
            summary_df = pd.read_csv(summary_output)
            print("\nSample of Danish tracks (most repeated first):")
            print(summary_df.head(10).to_string(index=False))
            print("\nTotal unique Danish tracks: {}".format(len(summary_df)))
        except Exception as e:
